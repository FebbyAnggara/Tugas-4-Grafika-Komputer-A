...
void Demo::BuildTexturedCube() {
	...
	int width, height;
	unsigned char* image = SOIL_load_image("speaker0.png", &width, &height, 0, SOIL_LOAD_RGBA);
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	image = SOIL_load_image("speakerterangg.png", &width, &height, 0, SOIL_LOAD_RGBA);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, image);
	SOIL_free_image_data(image);
	glBindTexture(GL_TEXTURE_2D, 0);
	...
	float vertices[] = {
		...
		0.5, -0.5, 0.5, 0.25, 0, 0.0f,  0.0f,  1.0f,  // 1
		0.5,  1.2, 0.5, 0.25, 1, 0.0f,  0.0f,  1.0f,  // 2
		-0.5,  1.2, 0.5, 0, 1, 0.0f,  0.0f,  1.0f, // 3

		// right
		0.5,  -0.5,  -0.5, 0.25, 0, 1.0f,  0.0f,  0.0f, // 4
		0.5,  -0.5, 0.5, 0.5, 0, 1.0f,  0.0f,  0.0f, // 5
		0.5, 1.2, 0.5, 0.5, 1, 1.0f,  0.0f,  0.0f, // 6
		0.5, 1.2,  -0.5, 0.25, 1, 1.0f,  0.0f,  0.0f, // 7

		// back
		-0.5, -0.5, -0.5, 0.5, 0, 0.0f,  0.0f,  -1.0f, // 8 
		0.5,  -0.5, -0.5, 0.75, 0, 0.0f,  0.0f,  -1.0f,// 9
		0.5,   1.2, -0.5, 0.75, 1, 0.0f,  0.0f,  -1.0f,// 10
		-0.5,  1.2, -0.5, 0.5, 1, 0.0f,  0.0f,  -1.0f, // 11

		// left
		-0.5, -0.5, -0.5, 0.25, 0, -1.0f,  0.0f,  0.0f, // 12
		-0.5, -0.5,  0.5, 0.5, 0, -1.0f,  0.0f,  0.0f, // 13
		-0.5,  1.2,  0.5, 0.5, 1, -1.0f,  0.0f,  0.0f, // 14
		-0.5,  1.2, -0.5, 0.25, 1, -1.0f,  0.0f,  0.0f, // 15

		// upper
		0.5, 1.2,  0.5, 0.75, 0, 0.0f,  1.0f,  0.0f,  // 16
		-0.5, 1.2,  0.5, 1, 0, 0.0f,  1.0f,  0.0f, // 17
		-0.5, 1.2, -0.5, 1, 1, 0.0f,  1.0f,  0.0f, // 18
		0.5, 1.2, -0.5, 0.75, 1, 0.0f,  1.0f,  0.0f,  // 19

		// bottom
		-0.5, -0.5, -0.5, 0.75, 0, 0.0f,  -1.0f,  0.0f, // 20
		0.5, -0.5, -0.5, 1, 0, 0.0f,  -1.0f,  0.0f, // 21
		0.5, -0.5,  0.5, 1, 1, 0.0f,  -1.0f,  0.0f, // 22
		-0.5, -0.5,  0.5, 0.75, 1, 0.0f,  -1.0f,  0.0f, // 23
	};
...
void Demo::BuildTexturedPlane()
{
	...
	int width, height;
	unsigned char* image = SOIL_load_image("lantai1.png", &width, &height, 0, SOIL_LOAD_RGBA);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, image);
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	image = SOIL_load_image("lantaispecularmap.png", &width, &height, 0, SOIL_LOAD_RGBA);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, image);
	SOIL_free_image_data(image);
	glBindTexture(GL_TEXTURE_2D, 0);
