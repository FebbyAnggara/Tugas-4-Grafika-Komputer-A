...
void Demo::Render() {
   ...
   // ----------------------------------------
	...
	lightView = glm::lookAt(glm::vec3(-2.0f, 0.5f, -1.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0, 1.0, 0.0)); // -2.0f, 4.0f, -1.0f
	...
	glUniform3f(glGetUniformLocation(this->shadowmapShader, "lightPos"), -2.0f, 0.5f, -1.0f);
...
// Load and create a texture 
	...
	int width, height;
	unsigned char* image = SOIL_load_image("speakerframe.png", &width, &height, 0, SOIL_LOAD_RGBA);
	...
	float vertices[] = {
		...
		0.5,  1.2, 0.5, 1, 1, 0.0f,  0.0f,  1.0f,  // 2
		-0.5,  1.2, 0.5, 0, 1, 0.0f,  0.0f,  1.0f, // 3

		// right
		0.5,  -0.5,  -0.5, 0, 0, 1.0f,  0.0f,  0.0f, // 4
		0.5,  -0.5,   0.5, 1, 0, 1.0f,  0.0f,  0.0f, // 5
		0.5,   1.2,   0.5, 1, 1, 1.0f,  0.0f,  0.0f, // 6
		0.5,   1.2,  -0.5, 0, 1, 1.0f,  0.0f,  0.0f, // 7

		// back
		-0.5, -0.5, -0.5, 0, 0, 0.0f,  0.0f,  -1.0f, // 8 
		0.5,  -0.5, -0.5, 1, 0, 0.0f,  0.0f,  -1.0f,// 9
		0.5,   1.2, -0.5, 1, 1, 0.0f,  0.0f,  -1.0f,// 10
		-0.5,  1.2, -0.5, 0, 1, 0.0f,  0.0f,  -1.0f, // 11

		// left
		-0.5, -0.5, -0.5, 0, 0, -1.0f,  0.0f,  0.0f, // 12
		-0.5, -0.5,  0.5, 1, 0, -1.0f,  0.0f,  0.0f, // 13
		-0.5,  1.2,  0.5, 1, 1, -1.0f,  0.0f,  0.0f, // 14
		-0.5,  1.2, -0.5, 0, 1, -1.0f,  0.0f,  0.0f, // 15

		// upper
		0.5, 1.2,  0.5, 0, 0, 0.0f,  1.0f,  0.0f,  // 16
		-0.5, 1.2,  0.5, 1, 0, 0.0f,  1.0f,  0.0f, // 17
		-0.5, 1.2, -0.5, 1, 1, 0.0f,  1.0f,  0.0f, // 18
		0.5, 1.2, -0.5, 0, 1, 0.0f,  1.0f,  0.0f,  // 19
